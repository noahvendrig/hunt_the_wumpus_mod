...
class WumpusGame(object):
    def __init__(self, edges=[]):
        ...
        self.arrow_travel_distance = (
            5  # As in the original game. I don't like this choice:
        )
    ...
    def breadth_first_search(self, source, target, max_depth=5):
        ...
        def search(stack, visited, target, depth):
            ...
            stack = list(
                set(
                    [graph[v][i] for v in stack for i in range(len(graph[v]))]
                ).difference(visited)
            )
            depth += 1
            ...
            if mode == "m":
                ...
                try:  # When shooting, the target must be reachable within 5 tunnels.
                    ...
                    if bfs[1] == -1:  # The target is outside cave.
                        print(
                            "There is no room with this number in the cave. Your arrow travels randomly."
                        )
    ...
    def shoot_room(self, room_number):
        ...
        if (
            self.arrows < 1
        ):  # This (or the updating of self.arrows) seems to be broken...
        ...
        if random.random() < 0.75:
            ...
            new_pos = random.choice(
                list(set(self.cave[wumpus_pos]).difference(self.threats.keys()))
            )
            del self.threats[room_number]
